              ...
              <div class="attentionerror">
			   ...
			   if(isset($space))
			   {
                 ...
                 <div class="myspace_list_inner row">
                 	<div class="col-md-8">
                    	<h3><?php echo $s->typeofspace; ?> on <?php echo $s->address.', '.$s->city; ?>, <?php echo $s->zipcode; ?> <span class="price_space pull-right">$<?=$s->pday?>.00 per day</span></h3>
                        <div class="row">
                        <div class="col-md-9">
                        ...
                        <div class="grid_images_prk grid">
                           <div class="row">
							<?php if(!empty($s->photos)){
								foreach($s->photos as $photo){?>
                            <div class="col-sm-3">
                              <img src="<?php echo base_url(); ?>upload/space/<?php echo $photo; ?>" class="img-responsive">
                            </div>
							<?php } } ?>				
                          </div>
                        </div>
					...
					?>
					<div class="col-md-4"><!--<iframe style="width:100%;" frameborder="0" id="cusmap" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.it/maps?q=<?php echo $address; ?>&output=embed"></iframe>-->
						<div class="frst_map">
						<div id="map<?=$s->id?>" style="width:200px;height:150px; margin: 0 auto;"></div>
          </div>
            <div class="scnd_map">
						<div id="pano<?=$s->id?>" style="width:100%;height:100px;"></div>
          </div>
					</div>
                 </div>
				 <script type="text/javascript">
					function initialize<?=$s->id?>() {
						var latlng = new google.maps.LatLng(<?php  echo $s->lat; ?>,<?php  echo $s->lng; ?>);
						var map = new google.maps.Map(document.getElementById('map<?=$s->id?>'), {
						  center: latlng,
						  zoom: 10,
						 ...
						 var panorama = new google.maps.StreetViewPanorama(
							document.getElementById('pano<?=$s->id?>'), {
							  position: latlng,
							  pov: {
								heading: 34,
								pitch: 10
					...
					google.maps.event.addDomListener(window, 'load', initialize<?=$s->id?>);
					</script>
			   <?php }} ?>
