...
<div class="clearfix"></div>
 <form action="<?php echo base_url(); ?>Dashboard/updaterentourspace/<?php echo $this->uri->segment(2); ?>" id="valid-form" class="" method="post" enctype="multipart/form-data">
        ...
        <div class="dash_dtl_frm">
               ...
               <div class="credit">
                <div class="row bs-example-popover">
				  ...
				  <br>
                  <p><strong>Note</strong>: You can drag n drop icon, if you think address is not correct.</p>
                  <br>
				  ...
				  <br>
                        ...
                        <div class="row">
                           ...
                           <div class="col-md-4">
                              ...
                              <div class="form-group">
                                 <input type="text" class="form-control" id="state" placeholder="State" name="state" value="<?php if(isset($space[0]->state)) { echo $space[0]->state; } ?>">
                   ...
                   <div class="dtl-Sec wdh">
                      ...
                      <div class="dtl-Sec wdh">
                        ...
                        <div class="row">
                           <div class="col-md-12">
                              <div class="credit">
                                 <h4><span>Near By Places <small>( Add nearest places like Airport, Station, Hotels etc. )</small></span></h4>
											<br>
												<input type="button" class="btn btn-success" id="add_more_milestone" value="Add Places"/><small> (You can add upto 5 places)</small>
												<input type="hidden" name="milestone_field" id="milestone_field" value="<?php echo count(json_decode($space[0]->nearby)) + 1; ?>"/>
												
						<?php if(!empty($space[0]->nearby) && ($space[0]->nearby != 'null')) { foreach(json_decode($space[0]->nearby) as $key=>$value) {?>												
							<div class='row'>
							<div class='col-sm-3'>
							<div class='form-group'>
							<label class='control-label'>Place <?=$key+1?></label>
							<input type='text'  class='form-control milestone' id='"+img_count+"' name='nearby[]' placeholder='Place Name' value="<?=$value?>"/>
							</div>
							</div>
							...
							<input type='number' min='1' class='form-control milestone' id='"+img_count+"' name='nearbydistance[]' placeholder='Distance' value="<?php print_r(json_decode($space[0]->nearbydistance)[$key]); ?>"/>
							</div>
							</div>
							...
							<label class='control-label'>&nbsp;</label>
							<div class='form-group'>
							<i class='fa fa-trash remove_img_div'></i>
							</div>
							</div>
							</div>
							<?php } } ?>
...
<script type="text/javascript">
function initialize() {
    var latlng = new google.maps.LatLng(<?php  echo $space[0]->lat; ?>,<?php  echo $space[0]->lng; ?>);
    var map = new google.maps.Map(document.getElementById('map'), {
	 ...
	 var locations = [["<?=$space[0]->typeofspace?>, <?=$space[0]->address?>", "<?=$space[0]->lat?>", "<?=$space[0]->lng?>", 5]];
